...
def columns_average(dataframe, variable):
...
for col in purchase_columns:
    print(f'{col}_average: {columns_average(df, col)}')

click_columns = [col for col in df.columns if 'click' in col]
for col in click_columns:
    print(f'{col}_average: {columns_average(df, col)}')

impression_columns = [col for col in df.columns if 'impression' in col]
for col in impression_columns:
    print(f'{col}_average: {columns_average(df, col)}')

earning_columns = [col for col in df.columns if 'earning' in col]
...
click_control_average: 5100.66
click_test_average: 3967.55
impression_control_average: 101711.45
impression_test_average: 120512.41
earning_control_average: 1908.57
earning_test_average: 2514.89
"""
...
""" 
p-value_impression_control: 0.3514
test_stat_impression_control: 0.9697
p-value_impression_test: 0.4148
test_stat_impression_test: 0.972
"""
for col in click_columns:
    print(f'p-value_{col}: {round(normal_assumption(df, col)[1], 4)}')
    print(f'test_stat_{col}: {round(normal_assumption(df, col)[0], 4)}')
...
""" 
p-value_click_control: 0.8461
test_stat_click_control: 0.9844
p-value_click_test: 0.9699
test_stat_click_test: 0.9896
"""
...
""" 
p-value_earning_control: 0.5306
test_stat_earning_control: 0.9756
p-value_earning_test: 0.6163
test_stat_earning_test: 0.978
"""

"""
Since p-values for all variables are higher than 0.05, Ho hypotheses can not rejected.
Therefore, the normal distribution is provided and there is no statistically significant difference between the mean values
"""
...
# Test Stat: 2.6393 and p-value: 0.1083 for purchase

test_stat, pvalue = variance_homogenity(df, 'impression_control', 'impression_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: 0.5865 and p-value: 0.4461 for impression

test_stat, pvalue = variance_homogenity(df, 'click_control', 'click_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: 6.3041 and p-value: 0.0141 for click

test_stat, pvalue = variance_homogenity(df, 'earning_control', 'earning_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: 0.3532 and p-value: 0.554 for earning

"""
Since p-values>0.05, except for the click variable, Ho hypothesis can not rejected.
Therefore, the variance homogenity assumption is provided and 
there is no statistically significant difference between the mean values of variables
"""
...
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: -0.9416 and p-value: 0.3493 for purchase

test_stat, pvalue = parametric_test(df, 'impression_control', 'impression_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: -4.2966 and p-value: 0.0 for impression

test_stat, pvalue = parametric_test(df, 'click_control', 'click_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: 4.4266 and p-value: 0.0 for click

test_stat, pvalue = parametric_test(df, 'earning_control', 'earning_test')
print(f'Test Stat: {round(test_stat, 4)} and p-value: {round(pvalue, 4)}')
# Test Stat: -9.2545 and p-value: 0.0 for earning
...
Test Stat: 4.4266 and p-value: 0.0 for click
Test Stat: -9.2545 and p-value: 0.0 for earning
Since p-value=0.3493>0.05 for purchase, H0 hypothesis can not rejected.
Since p-value=0.0<0.05 for impression, click, and earning, H0 hypothesis is rejected.
Therefore, there is no statistically significant difference between the mean values of purchase_control and purchase_test.
Therefore, there is a statistically significant difference between the mean values of click, impression, and earning.
Although the difference between mean values for purchase is a chance, others not. 
"""
